				...
				if(rSearch.isEmpty()){
					...
					if(aSearch.isEmpty()){
						...
						if(request.getParameterMap().isEmpty()){
							RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/Views/Homeworks/add.jsp");
							rd.forward(request, response);
						}
						else{
							Homework a = new Homework(
								(String)request.getParameter("alumno"),
								(String)request.getParameter("tema"),
								(String)request.getParameter("curso"),
								(String)request.getParameter("profesor"));
