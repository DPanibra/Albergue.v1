				...
				if(rSearch.isEmpty()){
					...
					if(aSearch.isEmpty()){
						...
						try{
							...
							List<Access> listAccess = (List<Access>) q.execute();
							request.setAttribute("listaAccess", listAccess);
							RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/Views/Access/index.jsp");
							rd.forward(request, response);
						}catch(Exception e){
